...
from control.mppi_torch.mppi_torch.mppi_utils import generate_gaussian_halton_samples, scale_ctrl, cost_to_go
...
class MPPIPlanner(ABC):
    ...
    def __init__(self, cfg: Union[MPPIConfig, dict], nx: int, dynamics: Callable, running_cost: Callable, prior: Optional[Callable] = None):
        ...
        self.mean_action = torch.zeros(
            self.nu, device=self.tensor_args['device'], dtype=self.tensor_args['dtype'])
        ...
        self.gamma_seq = torch.cumprod(torch.tensor(
            [1.0] + [self.gamma] * (self.T - 1)), dim=0).reshape(1, self.T)
    ...
    def get_samples(self, sample_shape, **kwargs):
        ...
        if (self.sample_method == 'halton'):
            ...
            # n knots is T/knot_scale (30/4 = 7)
            knot_samples = self.knot_points.view(
                sample_shape, self.nu, self.n_knots)
            self.samples = torch.zeros(
                (sample_shape, self.T, self.nu), **self.tensor_args)
    ...
    def _compute_rollout_costs(self, perturbed_actions):
        ...
        cost_total = torch.zeros(
            K, device=self.tensor_args['device'], dtype=self.tensor_args['dtype'])
        cost_horizon = torch.zeros(
            [K, T], device=self.tensor_args['device'], dtype=self.tensor_args['dtype'])
        ...
        for t in range(T):
            ...
            u.to(dtype=self.tensor_args['dtype'],
                 device=self.tensor_args['device'])
    ...
    def _update_distribution(self, costs, actions):
        ...
        weighted_seq = w * torch.transpose(actions, 0, 2)
        weighted_seq_trans = torch.transpose(weighted_seq, 0, 2)

        new_mean = torch.sum(weighted_seq_trans, dim=0)

        # Gradient update for the mean
        self.mean_action = (1.0 - self.step_size_mean) * \
        ...
        if self.update_cov:
            ...
            weighted_delta = w * torch.transpose(delta ** 2, 0, 2)
            weighted_delta_trans = torch.transpose(weighted_delta, 0, 2)
            cov_update = torch.mean(
                torch.sum(weighted_delta_trans, dim=0), dim=0)

            self.cov_action = (1.0 - self.step_size_cov) * \
    ...
    def get_action_cost(self):
        if self.noise_abs_cost:
            action_cost = self.lambda_ * \
                torch.abs(self.noise) @ self.noise_sigma_inv
    ...
    def _compute_total_cost_batch_halton(self):
        ...
        scaled_delta = torch.matmul(self.delta, torch.diag(
            self.scale_tril)).view(self.delta.shape[0], self.T, self.nu)
        ...
        act_seq = scale_ctrl(act_seq, self.u_min,
                             self.u_max, squash_fn=self.squash_fn)
